...
class Wraith(FlyableAttackUnit): # FlyableAttackUnit 상속
    ...
    def clocking(self):
        if self.clocked == True:    # 클로킹 모드 -> 모드 해제
            ...
            print("{0} : ㅋ 클로킹 모드 설정합니다.".format(self.name))
            self.clocked = True
...
for unit in attack_units:
    if isinstance(unit, Marine):    # unit이 Marine 인지 물어봅니다.
        unit.stimpack()
    elif isinstance(unit, Tank):    # unit이 Tank 인지 물어봅니다.
        unit.set_seize_mode()
    elif isinstance(unit, Wraith):  # unit이 Wraith 인지 물어봅니다.
        unit.clocking()
